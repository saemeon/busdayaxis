...
high_i = np.maximum(open_i.values, close_i.values) + rng2.uniform(0.01, 0.08, len(bar_times))
low_i = np.minimum(open_i.values, close_i.values) - rng2.uniform(0.01, 0.08, len(bar_times))
...
full_days_i = pd.date_range(bar_times.min().normalize(), bar_times.max().normalize(), freq="D")
...
for d in full_days_i:
    ax5.axvspan(d, d + pd.Timedelta(hours=OPEN_H), color="grey", alpha=0.15, linewidth=0)
    ax5.axvspan(
...
for d in full_days_i:
    ax6.axvline(d + pd.Timedelta(hours=OPEN_H), linestyle="--", linewidth=0.8, alpha=0.6)
    ax6.axvline(d + pd.Timedelta(hours=CLOSE_H), linestyle="--", linewidth=0.8, alpha=0.6)
